...
const getGameOwnerProfileFromLocalStorage = () => localStorage.getItem(PLAYER_LOCAL_STORAGE_KEY)

const setGameOwnerProfileToLocalStorage = (gameOwnerProfile: GameOwnerProfile | undefined) => {
    if (gameOwnerProfile) localStorage.setItem(PLAYER_LOCAL_STORAGE_KEY, JSON.stringify(gameOwnerProfile))
    else localStorage.removeItem(PLAYER_LOCAL_STORAGE_KEY)
}

export const gameOwnerProfile: Writable<GameOwnerProfile | undefined> = writable()
...
export const processAfterGoogleLogin = (gameOwnerProfileFromIdp: GoogleAccountProfile) => {
